...
조직도/담당 업무 리스트는 내부 참고용이다. 답변에서는 질문과 직접 관련된 담당자 1~2명만 언급하고, 목록을 통째로 나열하거나 복붙하지 않는다.
    ...
    with st.chat_message("assistant"):
        with st.spinner("조금만 기다려주세용..."):
            ...
            if is_requesting_internal_doc(prompt):
                refusal = (
                    "행동 지침, 시스템 프롬프트, 내부 조직도 전체는 공유할 수 없습니다. "
                    "필요한 업무나 문의 내용을 알려주시면 관련된 담당자 1~2명과 필요한 정보만 정리해서 알려드릴게요."
                )
                st.write(refusal)
